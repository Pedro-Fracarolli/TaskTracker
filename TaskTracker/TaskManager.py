...
class TaskManager:
    ...
    def SaveTasks(self):
        ...
        tasks_json = json.dumps(tasks_as_dicts, indent = 4)
        with open("TaskList.json", "w") as save_file:
            save_file.write(tasks_json)

    def ImportTasks(self):
        global tasks_list
        with open("TaskList.json", "r") as imported_file:
            imported_json = json.load(imported_file)
            tasks_list = [Task(description=task["description"], status=task["status"], createdAt=task.get("createdAt"))
                          for task in imported_json]
